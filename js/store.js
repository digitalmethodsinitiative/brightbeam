const store = {
  ...
  async getTrackerList() {
      let trackerList;
      try {
        trackerList = await fetch(this.TRACKERLIST_URL);
        trackerList = await trackerList.json();
        trackerList = trackerList["bugs"].map((tracker) => { return tracker['pattern']; });
      } catch (error) {
        console.log(error);
        trackerList = {};
      }
...
*/
  ...
  outputWebsite(hostname, website) {
    const output = {
      ...
      thirdParties: [],
      requestedTrackerURLs: [],
    };
    if ('thirdPartyHostnames' in website) {
      output.thirdParties = website.thirdPartyHostnames;
    }
    if ('requestedTrackerURLs' in website) {
      output.requestedTrackerURLs = website.requestedTrackerURLs;
    }
    return output;
  },
  ...
  */
  ...
  async setFirstParty(hostname, data) {
    ...
    if (false && isNewWebsite) {
      this.updateChild(this.outputWebsite(hostname, responseData));
    }
  },

  async setThirdParty(origin, target, data, url) {
    if (!origin) {
    ...
    if(!this.onAllowList(origin, target) && this.onTrackerList(url)) {
      await this.addFirstPartyTrackerURL(origin, url);
    } else {
      await this.addFirstPartyTrackerURL(origin, target);
    }
  ...
  async addFirstPartyTrackerURL(firstPartyHostname, trackerURL) {
    const firstParty = await this.getWebsite(firstPartyHostname);
    // We are likely storing the tp first, lets account for that
    if (!('firstParty' in firstParty)) {
      firstParty.firstParty = true;
    }
    if (!('requestedTrackerURLs' in firstParty)) {
      firstParty['requestedTrackerURLs'] = [];
    }
    if (!firstParty['requestedTrackerURLs'].includes(trackerURL)) {
      firstParty['requestedTrackerURLs'].push(trackerURL);
      await this.setFirstParty(firstPartyHostname, firstParty);
    }
  },
